...
const PostEditor = () => {
  ...
  const fetchPost = async () => {
    ...
    try {
      ...
      // apiRequest already returns JSON data directly
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // apiRequest returns JSON data directly, not a Response object
      const data = await apiRequest(url, {
        method,
        body: JSON.stringify(postData),
      });

      // Since apiRequest already handles the response and returns JSON,
      // we just need to check if we got data back successfully
